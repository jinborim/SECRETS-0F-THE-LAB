...
public class CabinetOpen_Y : MonoBehaviour, IPointerClickHandler
{
    ...
    void Start()
    {
        ...
        message = new string[] { "오래된 것인지 회전축에 녹이 쓸었다. 뻑뻑해서 열 수가 없다.", "이상하게 몸에 힘이 들어가지 않아 힘으로 열 수도 없다.", "뭔가 '윤활제' 같은 게 있다면..." };
    }
        ...
        if (eventData.button == PointerEventData.InputButton.Right)
        {
            ...
            if (Physics.Raycast(ray, out hit) && hit.transform.name == "door")
            {
                ...
                else if (is_locked == true)
                {
                    ...
                    else if(Inventory_Checking.Item_Use(lockKey.Key) != true)
                    {
